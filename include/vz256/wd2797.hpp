...
private:
    enum class Transfer { none, read, write, read_address, read_track, write_track };
    enum class FormatState { search_id, id, search_data, data, data_crc };
    ...
    bool begin_write_track(std::array<FloppyImage, 4>& drives, std::uint8_t drive);
    ...
    std::vector<std::uint8_t> format_data_;
    std::vector<bool> formatted_sector_seen_;
    std::size_t formatted_sectors_{};
};
